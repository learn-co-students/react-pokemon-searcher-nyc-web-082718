...
class PokemonForm extends React.Component {
  ...
  handleSubmit = (event) => {
    ...
    const inputFields = event.target.children[0].children
    // debugger
    const newName = inputFields[0].children[1].children[0].value
    const newHp = inputFields[1].children[1].children[0].value
    const newFront = inputFields[2].children[1].children[0].value
    const newBack = inputFields[3].children[1].children[0].value
    this.props.createNewPokemon(newName, newHp, newFront, newBack)
    event.target.reset()
  }
